...
class AgentManager {
  constructor({ store, dataDir, broadcast }) {
  ...
  update(id, patch) {
    ...
    const allowed = ["name", "model", "cwd", "apiKeyId"];
  ...
  _spawnClaude(agent, prompt) {
    ...
    this._attachProc(agent, proc, args, /*retriedWithShell=*/ resolved.needsShell);
  }

  _attachProc(agent, proc, args, retriedWithShell) {
    const id = agent.id;
    ...
    proc.on("close", (code) => this._onClose(agent, code, proc));
  }
  ...
  _spawnClaudeWithShell(agent, args) {
    ...
    this._attachProc(agent, proc, args, /*retriedWithShell=*/ true);
  }
